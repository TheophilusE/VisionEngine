...
InputManager::InputManager() :
    m_GainputMap(m_GainputManager), m_Listener(BoolListen, FloatListen), m_Width(0), m_Height(0)
{
}
...
{
    j["InputDevice"] = (int)s.device;
    j["InputButton"] = (int)s.button;
    j["InputEvent"]  = s.inputEvent;
}

void FROM_JSON(const JSON::json& j, InputDescription& s)
{
    s.device     = (Device)(int)j["InputDevice"];
    s.button     = (int)j["InputButton"];
