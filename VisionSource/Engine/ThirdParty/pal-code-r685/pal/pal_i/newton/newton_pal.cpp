...
static int CDECL GenericContactBegin(const NewtonMaterial* material, const NewtonBody* body0,
...
static void CDECL GenericContactProcess(const NewtonJoint* contact, dFloat timestep, int threadIndex) {
	float pos[3];
	float norm[3];
...
float CDECL RayCastPlacement(const NewtonBody* body, const float* normal, int collisionID,
			void* userData, float intersetParam) {
	float range = 0;
	palVector3 dir;
	palVector3 pos;
	...
	if (intersetParam < pickedParam) {
		...
		if (prh) {
			palNewtonBodyData* d =
						static_cast<palNewtonBodyData *> (NewtonBodyGetUserData(pickedBody));
			if (d) {
				prh->m_pBody = d->pb;
			}
...
unsigned CDECL newtonRaycastPreFilter(const NewtonBody *body, const NewtonCollision *collision,
			void* userData) {
	return 1;
}
...
void palNewtonMaterialInteraction::Init(palMaterial *pM1, palMaterial *pM2,
	...
	NewtonMaterialSetCollisionCallback(g_nWorld, pNM1->m_GroupID, pNM2->m_GroupID, NULL,
				GenericContactBegin, GenericContactProcess);
}
...
void palNewtonPhysics::InitWater(Float fluidDensity, Float fluidLinearViscosity,
			Float fluidAngularViscosity, Float plane_a, Float plane_b, Float plane_c, Float plane_d) {
	g_water = true;
...
int CDECL mygetbuoyancyplane(const int collisionID, void *context, const float* globalSpaceMatrix,
...
static void CDECL PhysicsApplyForceAndTorque(const NewtonBody* body, dFloat timestep, int threadIndex) {
	float mass;
	float Ixx;
	float Iyy;
	float Izz;
...
const char* palNewtonPhysics::GetPALVersion() const {
	static char verbuf[512];
	sprintf(verbuf,
				"PAL SDK V%d.%d.%d\nPAL Newton V:%d.%d.%d\nFile: %s\nCompiled: %s %s\nModified:%s",
				PAL_SDK_VERSION_MAJOR, PAL_SDK_VERSION_MINOR, PAL_SDK_VERSION_BUGFIX,
				NEWTON_PAL_SDK_VERSION_MAJOR, NEWTON_PAL_SDK_VERSION_MINOR,
				NEWTON_PAL_SDK_VERSION_BUGFIX, __FILE__, __TIME__, __DATE__, __TIMESTAMP__);
	return verbuf;
}

const char* palNewtonPhysics::GetVersion() const {
...
void palNewtonPhysics::Init(const palPhysicsDesc& desc) {
	...
	NewtonMaterialSetCollisionCallback(g_nWorld, defaultgroup, defaultgroup, NULL,
				GenericContactBegin, GenericContactProcess);
}
;
...
void palNewtonPhysics::RayCast(Float x, Float y, Float z, Float dx, Float dy, Float dz,
			Float range, palRayHit& hit) {
	float pos0[3];
	float pos1[3];
...
void palNewtonPhysics::GetContacts(palBodyBase *pBody, palContact& contact) const {
	PAL_MAP<NewtonBody*, palContact> ::iterator itr;
	palNewtonBody *pb = dynamic_cast<palNewtonBody *>(pBody);
	itr=g_ContactsData.find(pb->m_pntnBody);
	if (itr!=g_ContactsData.end()) {
...
void palNewtonPhysics::GetContacts(palBodyBase *a, palBodyBase *b, palContact& contact) const {
	PAL_MAP<NewtonBody*, palContact> ::iterator itr;
...
void palNewtonBody::SetPosition(const palMatrix4x4 &location) {
	if (m_pntnBody) {
...
bool palNewtonBody::IsActive() const {
	return bool(NewtonBodyGetSleepState(m_pntnBody));
}

void palNewtonBody::SetActive(bool active) {
	if (!m_pntnBody)
		return;
	NewtonBodySetFreezeState(m_pntnBody, active ? 1 : 0);
}
...
void palNewtonBody::GetLinearVelocity(palVector3& velocity) const {
	NewtonBodyGetVelocity(m_pntnBody, velocity._vec);
}
void palNewtonBody::GetAngularVelocity(palVector3& velocity_rad) const {
	NewtonBodyGetOmega(m_pntnBody, velocity_rad._vec);
}

void palNewtonBody::SetLinearVelocity(const palVector3& velocity) {
	NewtonBodySetVelocity(m_pntnBody, velocity._vec);
}
void palNewtonBody::SetAngularVelocity(const palVector3& velocity_rad) {
...
const palMatrix4x4& palNewtonBody::GetLocationMatrix() const {
	if (m_pntnBody) {
		palMatrix4x4 m;
...
void palNewtonBoxGeometry::Init(const palMatrix4x4 &pos, Float width, Float height, Float depth,
								Float mass) {
    ...
    palMatrix4x4 matrixForNewton(pos);
	if (!pnb->static_body) {
		palVector3 bpos;
...
// This value based on Newton wiki: http://www.newtondynamics.com/wiki/index.php5?title=NewtonCreateConvexHull#The_tolerance_parameter
static dFloat TOLERANCE = 0.002;
...
void palNewtonConvex::Init(Float x, Float y, Float z, const Float *pVertices, int nVertices,
			Float mass) {
	...
	NewtonBodySetMassMatrix(m_pntnBody, m_fMass, png->m_fInertiaXX, png->m_fInertiaYY,
				png->m_fInertiaZZ);
...
void palNewtonBox::Init(Float x, Float y, Float z, Float width, Float height, Float depth,
			Float mass) {
...
void palNewtonSphericalLink::Init(palBodyBase *parent, palBodyBase *child, Float x, Float y,
			Float z) {
...
Float palNewtonRevoluteLink::GetAngularVelocity() const {
...
void palNewtonRevoluteLink::Init(palBodyBase *parent, palBodyBase *child, Float x, Float y,
	...
	axis[1] = axis_y;
	axis[2] = axis_z;
	m_pntnJoint = NewtonConstraintCreateHinge(g_nWorld, pivot, axis, body1->m_pntnBody,
				body0->m_pntnBody);
...
void palNewtonPrismaticLink::Init(palBodyBase *parent, palBodyBase *child, Float x, Float y,
			Float z, Float axis_x, Float axis_y, Float axis_z) {
	...
	axis[1] = axis_y;
	axis[2] = axis_z;
	m_pntnJoint = NewtonConstraintCreateSlider(g_nWorld, pivot, axis, body1->m_pntnBody,
				body0->m_pntnBody);
}
...
const palMatrix4x4& palNewtonTerrain::GetLocationMatrix() const {
	if (m_pntnBody) {
		palMatrix4x4 m;
		NewtonBodyGetMatrix(m_pntnBody, m._mat);
		memcpy(m_mLoc._mat, m._mat, sizeof(palMatrix4x4));
...
void palNewtonOrientatedTerrainPlane::Init(Float x, Float y, Float z, Float nx, Float ny, Float nz,
			Float min_size) {
...
const palMatrix4x4& palNewtonOrientatedTerrainPlane::GetLocationMatrix() const {
...
void palNewtonTerrainMesh::Init(Float px, Float py, Float pz, const Float *pVertices,
			int nVertices, const int *pIndices, int nIndices) {
...
const palMatrix4x4& palNewtonTerrainHeightmap::GetLocationMatrix() const {
	return palNewtonTerrainMesh::GetLocationMatrix();
}

void palNewtonTerrainHeightmap::Init(Float px, Float py, Float pz, Float width, Float depth,
...
void palNewtonPSDSensor::Init(palBody *body, Float x, Float y, Float z, Float dx, Float dy,
...
Float palNewtonPSDSensor::GetDistance() const {
	float pos0[3];
	float pos1[3];
...
void palNewtonContactSensor::GetContactPosition(palVector3& contact) const {
...
void palNewtonForceActuator::Init(palBody *pbody, Float px, Float py, Float pz, Float axis_x,
			Float axis_y, Float axis_z) {
	palNewtonBody *pnb = dynamic_cast<palNewtonBody *> (pbody);
	NewtonBodySetAutoSleep(pnb->m_pntnBody, 0);
